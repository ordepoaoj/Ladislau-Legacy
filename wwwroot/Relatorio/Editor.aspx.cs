...
public partial class Relatorio_Editor : System.Web.UI.Page
{
    ...
    public void ExportPDFClick(object sender, EventArgs e)
    {
        if (EscolhaBox.SelectedIndex == 0)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                {
                    PdfPTable pdftable = new PdfPTable(BrasilView.HeaderRow.Cells.Count);
                    pdftable.HeaderRows = 1;

                    foreach (TableCell headerCell in BrasilView.HeaderRow.Cells)
                    {
                        Font font = new Font();
                        font.Color = new BaseColor(BrasilView.HeaderStyle.ForeColor);
                        BrasilView.Attributes["style"] = "border-collapse:separate";


                        PdfPCell pdfPCell = new PdfPCell(new Phrase(headerCell.Text, font));
                        pdfPCell.BackgroundColor = new BaseColor(BrasilView.HeaderStyle.BackColor);
                        pdftable.AddCell(pdfPCell);

                    }

                    foreach (GridViewRow gridViewRow in BrasilView.Rows)
                    {
                        foreach (TableCell tableCell in gridViewRow.Cells)
                        {
                            Font font = new Font();
                            font.Color = new BaseColor(BrasilView.RowStyle.ForeColor);
                            BrasilView.Attributes["style"] = "border-collapse:separate";
                            PdfPCell pdfPCell = new PdfPCell(new Phrase(tableCell.Text));
                            pdfPCell.BackgroundColor = new BaseColor(BrasilView.RowStyle.BackColor);
                            pdftable.AddCell(pdfPCell);
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename=Brasil.pdf");
    ...
    protected void EscolhaBox_SelectedIndexChanged(object sender, EventArgs e)
    {
